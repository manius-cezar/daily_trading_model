...
def create_features(df: pd.DataFrame, window_length: int, pred_window_length: int, 
    ...
    close_data = df['Close']
    time_indicator = df['time_indicator'].to_numpy()
    # EMA13 calculation
    ...
    for i in range(0, n - c):
        ...
        statistics.append(time_indicator[i])
...
def segmentate_data(file_path: str, window_length: int, pred_window_length: int, 
    ...
    if len(df) < window_length:
        print('df len is < window_length')
        return None, None, None

    features: Iterable[Tuple[np.ndarray, np.ndarray]] = create_features(df, window_length, pred_window_length, 
    ...
    for file_path in files:
        if file_path.endswith(".csv"):
            ...
            X, y, mean_model_features = segmentate_data(os.path.join(dir, file_path), window_length, pred_window_length, 
                use_old_y = use_old_y, ema_lengths=ema_lengths, ema_length_for_mean_model=ema_length_for_mean_model)
            if X is None and y is None and mean_model_features is None:
                print('X, y, mean_model_features are None - skipping.')
                continue
            else:
                total_X.extend(X)
                total_y.extend(y)
